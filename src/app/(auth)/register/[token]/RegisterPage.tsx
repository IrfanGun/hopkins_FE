...
export default function RegisterPage({ token }: { token: string }) {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      try {
        ...
        setEmail(response.data.email); // asumsi backend mengembalikan { email: 'user@mail.com' }
      } catch (error: any) {
        const message = error.response?.data?.message || "Token tidak valid atau expired";
        setError(message);
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return <p className="p-8 text-center ">
